...
class IpWhitelistRiskEvaluatorTest {

    @Test
    void allInvalidEntriesReturnsInvalid() {
        var matcher = IpWhitelistMatcher.fromEntries(
                IpWhitelistEvaluatorConfig.splitEntries("not-an-ip,garbage,2001:db8::1,10.0.0.0/99"));
        var evaluator = new IpWhitelistRiskEvaluator(new FixedIpAddressContext(Optional.empty()));
        ...
        assertThat(risk.getReason().orElse(""), is("IP whitelist is not configured"));
    }
