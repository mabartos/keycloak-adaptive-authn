...
class IpWhitelistIndexTest {

    @Test
    void lookupUsesMergedRanges() {
        var index = IpWhitelistIndex.of(List.of(
                IpWhitelistMatcher.parseEntry("10.0.0.1-10.0.0.10").orElseThrow(),
                IpWhitelistMatcher.parseEntry("10.0.0.5-10.0.0.20").orElseThrow()));
    ...
    void emptyListReturnsNoMatch() {
        var index = IpWhitelistIndex.empty();
    ...
    void overlappingCidrAndRangeAreMerged() {
        var index = IpWhitelistIndex.of(List.of(
                IpWhitelistMatcher.parseEntry("10.0.0.0/24").orElseThrow(),
                IpWhitelistMatcher.parseEntry("10.0.0.128-10.0.0.255").orElseThrow()));
