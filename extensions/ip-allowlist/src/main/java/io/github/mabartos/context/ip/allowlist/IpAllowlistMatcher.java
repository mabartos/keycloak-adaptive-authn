...
public final class IpAllowlistMatcher {
    ...
    private static final Logger logger = Logger.getLogger(IpAllowlistMatcher.class);

    private final IpAllowlistIndex index;

    private IpAllowlistMatcher(IpAllowlistIndex index) {
        this.index = index;
    }

    public static IpAllowlistMatcher fromEntries(List<String> entries) {
        List<IpAllowlistEntry> parsed = new ArrayList<>();
        for (String entry : entries) {
            parseEntry(entry).ifPresent(parsed::add);
        }
        return new IpAllowlistMatcher(IpAllowlistIndex.of(parsed));
    }
    ...
    static Optional<IpAllowlistEntry> parseEntry(String entry) {
        if (entry == null) {
