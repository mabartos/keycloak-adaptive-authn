 ...
 * <ul>
 *   <li><strong>Free</strong> (no key): {@code http://ip-api.com/json/{ip}} — non-commercial, ~45 req/min per client IP.</li>
 *   <li><strong>Pro</strong> (API key): {@code https://pro.ip-api.com/json/{ip}?key=...} — SSL, commercial use.</li>
 * </ul>
 */
final class IpApiComGeoIpResolver implements GeoIpResolver {
    ...
    public Optional<LocationData> resolve(KeycloakSession session, RealmModel realm, IPAddress ip) {
        try {
            ...
            }).flatMap(this::toLocationOrEmpty);
    ...
    private Optional<LocationData> toLocationOrEmpty(IpApiComLocationData data) {
        if (data == null) {
            return Optional.empty();
        }
        if (!data.isSuccess()) {
