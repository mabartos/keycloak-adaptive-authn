    ...
    public Optional<LocationData> resolve(KeycloakSession session, RealmModel realm, IPAddress ip) {
        try {
            ...
            return GeoIpHttp.getJsonStream(session, uri).flatMap(stream -> {
                try {
                    IpApiLocationData parsed = JsonSerialization.readValue(stream, IpApiLocationData.class);
                    if (parsed != null && StringUtil.isNotBlank(parsed.getCountry())) {
                        return Optional.<LocationData>of(parsed);
                    }
                } catch (Exception e) {
